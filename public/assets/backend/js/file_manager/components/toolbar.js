(function (window) {
    ...
    window.FileManagerComponents.Toolbar = {
        ...
        template: [
            ...
            '  <div class="fm-title">',
            '    <h3>File Manager</h3>',
            '    <p>Browse FTP media, prepare photos, and select paths for form inputs.</p>',
            '  </div>',
            '  <div class="fm-toolbar-actions">',
            '    <label class="fm-search"><i class="mdi mdi-magnify"></i><input v-model="store.query" type="search" placeholder="Search files"></label>',
            ...
            '    <button type="button" class="fm-light-btn" @click="createFolder"><i class="mdi mdi-folder-plus-outline"></i><span>Folder</span></button>',
            '    <button type="button" class="fm-primary-btn" @click="store.setMode(\'upload\')"><i class="mdi mdi-cloud-upload-outline"></i><span>Upload</span></button>',
            '  </div>',
            '</header>'
        ].join('')
    };
